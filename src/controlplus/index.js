https://github.com/bricklife/scratch-lego-bluetooth-extensions/tree/master/scratch-vm/src/extensions/scratch3_controlplus
(async function (Scratch, require) {
    const BleBaseBlocks = await require('../legoble/ble-base-blocks.js');
    const Hub = await require('../legoble/hub.js');

    const blockIconURI = 'data:image/png;base64,';

    const formatMessage = Scratch.formatMessage;

    class Scratch3ControlPlusBlocks extends BleBaseBlocks {

        static get EXTENSION_ID () {
            return 'controlplus';
        }

        constructor () {
            super(new Hub(Scratch.vm.runtime, Scratch3ControlPlusBlocks.EXTENSION_ID, 0x80));
        }

        getInfo () {
            return {
                id: Scratch3ControlPlusBlocks.EXTENSION_ID,
                name: 'CONTROL+',
                extensionURL: Scratch3ControlPlusBlocks.extensionURL,
                blockIconURI: blockIconURI,
                showStatusButton: true,
                blocks: this.getBlocks(formatMessage),
                menus: this.getMenus(formatMessage)
            };
        }
    }

    Scratch.extensions.register(new Scratch3ControlPlusBlocks());
})(Scratch, require);
