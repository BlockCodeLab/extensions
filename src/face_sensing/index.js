(async function (Scratch, require) {
    await require('https://unpkg.com/@mediapipe/face_detection@0.4.1646425229/face_detection.js');
    await require('https://unpkg.com/@tensorflow/tfjs-core@4.4.0/dist/tf-core.min.js');
    await require('https://unpkg.com/@tensorflow/tfjs-backend-webgl@4.4.0/dist/tf-backend-webgl.min.js');
    await require('https://unpkg.com/@tensorflow-models/face-detection@1.0.1/dist/face-detection.min.js');
    ...
    class Scratch3FaceSensingBlocks {
        ...
        constructor () {
            ...
            const detectorConfig = {
                runtime: 'mediapipe',
                solutionPath: 'https://unpkg.com/@mediapipe/face_detection@0.4.1646425229/'
            };
        ...
        _loop () {
            ...
            if (frame) {
                this.detector.estimateFaces(frame, false).then(faces => {
                    if (faces) {
                        if (!this.firstTime) {
                            this.firstTime = true;
        ...
        faceSize () {
            ...
            if (!this.currentFace.box) return this.cachedSize;
            const size = Math.round(this.currentFace.box.width);
        ...
        updateAttachments () {
            ...
            this.runtime.targets.forEach(target => {
                const state = _this3._getFaceSensingState(target);

                if (state.attachedToPartNumber) {
                    const partPos = _this3.getPartPosition(state.attachedToPartNumber);
    ...
    Scratch.extensions.register(new Scratch3FaceSensingBlocks());
    ...
    Scratch.extensions.translations({
        ...
        'zh-cn': {
            'faceSensing.categoryName': '脸部侦测',
            'faceSensing.goToPart': '移到 [PART]',
            'faceSensing.pointInFaceTiltDirection': '面向脸部倾斜的方向',
            'faceSensing.setSizeToFaceSize': '将大小设为脸部大小',
            'faceSensing.whenTilted': '当脸部往 [DIRECTION] 倾斜',
            'faceSensing.whenSpriteTouchesPart': '当角色碰到 [PART]',
            'faceSensing.whenFaceDetected': '当检测到脸部',
            'faceSensing.faceDetected': '检测到脸部？',
            'faceSensing.faceTilt': '脸部倾斜方向',
            'faceSensing.faceSize': '脸部大小',
            'faceSensing.nose':'鼻子',
            'faceSensing.mouth':'嘴巴',
            'faceSensing.leftEye':'左眼',
            'faceSensing.rightEye':'右眼',
            'faceSensing.betweenEyes':'两眼之间',
            'faceSensing.leftEar':'左耳',
            'faceSensing.rightEar':'右耳',
            'faceSensing.topOfHead':'头顶',
            'faceSensing.left': '左',
            'faceSensing.right': '右'
        },
        'zh-tw': {
            'faceSensing.categoryName': '臉部偵測',
            'faceSensing.goToPart': '移到 [PART]',
            'faceSensing.pointInFaceTiltDirection': '面向臉部傾斜的方向',
            'faceSensing.setSizeToFaceSize': '將大小設為臉部大小',
            'faceSensing.whenTilted': '當臉部往 [DIRECTION] 傾斜',
            'faceSensing.whenSpriteTouchesPart': '當角色碰到 [PART]',
            'faceSensing.whenFaceDetected': '當檢測到臉部',
            'faceSensing.faceDetected': '檢測到臉部？',
            'faceSensing.faceTilt': '臉部傾斜方向',
            'faceSensing.faceSize': '臉部大小',
            'faceSensing.nose':'鼻子',
            'faceSensing.mouth':'嘴巴',
            'faceSensing.leftEye':'左眼',
            'faceSensing.rightEye':'右眼',
            'faceSensing.betweenEyes':'兩眼之間',
            'faceSensing.leftEar':'左耳',
            'faceSensing.rightEar':'右耳',
            'faceSensing.topOfHead':'頭頂',
            'faceSensing.left': '左',
            'faceSensing.right': '右'
        }
    });
