(async function (Scratch) {
    await Scratch.require('./style.css');
    await Scratch.require('./pixel-gallery.js', false);
    ...
    class PixelGalleryBlocks {
                ...
                didMount: element => {
                    if (element) {
                        ...
                        pixelGallery.addEventListener('open', e => {
                            ...
                            const data = this._getValue(id, name);
                            e.detail.resolve(data);
                        });
                        pixelGallery.addEventListener('save', e => {
                            const {id, name, data, width, height} = e.detail;
                            this._addKeyValue(id, name, {
                                data,
                                width,
                                height
                            });
        ...
        _updateList (list) {
            const id = list.id;
            list.value = Object.entries(this._gallery[id])
                .map(([key, value]) => `${encodeURIComponent(key)},${encodeURIComponent(value.data)},${value.width},${value.height}`);
        }
        ...
        isContains (args) {
            const id = Cast.toString(args.GALLERY);
            const key = Cast.toString(args.NAME);
            return !!(this._gallery[id] && this._gallery[id][key]);
        }
